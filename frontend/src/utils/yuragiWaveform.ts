...
export function generatePinkNoise(
  ...
  if (nSamples < 0) throw new Error('Number of samples must be non-negative')
  ...
  if (nSamples <= 4) {
    const mean = whiteNoise.reduce((a, b) => a + b, 0) / nSamples
    const centered = whiteNoise.map(x => x - mean)
    const std = nSamples > 1 ? Math.sqrt(centered.reduce((sum, x) => sum + x * x, 0) / (nSamples - 1)) : 1.0
    if (std > 0) {
      return centered.map(x => (x / std) * intensity)
    }
    return centered.map(x => x * intensity)
...
export function generateAmplitudeModulation(
  ...
  const envelope = t.map(time => 
    envelopeDepth * 0.7 * Math.sin(2 * Math.PI * envelopeFreq * time + 2 * Math.PI * 0.13)
  )
