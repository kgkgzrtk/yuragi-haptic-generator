...
class DirectionalFluctuationGenerator(ModulatorBase):
    ...
    def __init__(
        self,
        fluctuation_components: Optional[List[Dict[str, float]]] = None
    ):
        """
...
class NoiseGenerator:
    ...
    @staticmethod
    def pink_noise(n_samples: int, alpha: float = 1.0, seed: Optional[int] = None) -> np.ndarray:
        """
        ...
        if alpha < -2.0 or alpha > 5.0:
            raise ValueError(f"Alpha value {alpha} is outside reasonable range [-2.0, 5.0]")
        ...
        f_min = 1.0 / n_samples  # Minimum non-zero frequency (1 cycle over entire signal)
