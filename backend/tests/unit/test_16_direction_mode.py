...
class Test16DirectionMode:
    ...
    def test_device_has_16_direction_mode(self):
        ...
        assert hasattr(device, 'enable_16_direction_mode'), "Device should have 16-direction mode"
        assert hasattr(device, 'disable_16_direction_mode'), "Device should have disable method"
        assert hasattr(device, 'set_discrete_direction'), "Device should have discrete direction control"
    
    def test_16_direction_mode_disabled_by_default(self):
        """Test that 16-direction mode is disabled by default."""
        device = HapticDevice(sample_rate=44100)
        assert hasattr(device, 'discrete_mode_enabled'), "Device should have discrete_mode_enabled property"
        assert device.discrete_mode_enabled == False, "Discrete mode should be disabled by default"
    ...
    def test_set_discrete_direction(self):
        ...
        for idx in range(16):
            device.set_discrete_direction(device_id=1, direction_idx=idx, magnitude=1.0, frequency=30)
            ...
            assert params['amplitude'] == pytest.approx(expected_x_amp, abs=0.01), \
    ...
    def test_continuous_mode_after_discrete(self):
        ...
        device.set_vector_force(1, angle=47.3, magnitude=1.0, frequency=30)  # Arbitrary angle
    ...
    def test_16_directions_match_reference_angles(self):
        ...
        for idx, expected_angle in enumerate(reference_angles):
            ...
            assert actual_angle == pytest.approx(expected_angle, abs=0.1), \
    ...
    def test_16_direction_with_different_parameters(self):
        ...
        for direction_idx, magnitude, frequency in test_cases:
            ...
            expected_x_amp = magnitude * abs(np.cos(angle_rad))  # Amplitude is always positive
