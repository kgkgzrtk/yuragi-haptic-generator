...
class TestModulationEnvelopeAccuracy:
    ...
    def test_envelope_symmetry_and_distortion_characteristics(self):
        ...
        thd = np.sqrt(
            total_harmonic_power / fundamental_power
...
class TestModulationAmplitudeClipping:
    ...
    @pytest.mark.parametrize(
        "base_amplitude,min_clip,max_clip",
        [
    ...
    def test_clipping_boundaries_enforced_precisely(
        self, base_amplitude, min_clip, max_clip
    ):
    ...
    def test_clipping_preserves_signal_continuity(self):
        ...
        reasonable_max_slope = (
            10000  # Reasonable maximum slope for practical implementation
        )

        assert (
            max_derivative <= reasonable_max_slope
...
class TestModulationDepthEffects:
    ...
    @pytest.mark.parametrize(
        "depth,expected_peak_to_peak_ratio",
        [
    ...
    def test_modulation_depth_produces_expected_amplitude_variation(
        self, depth, expected_peak_to_peak_ratio
    ):
        ...
        assert (
            abs(peak_to_peak_ratio - expected_peak_to_peak_ratio) <= tolerance
    ...
    def test_modulation_depth_linearity_across_range(self):
        ...
        assert r_squared >= 0.95, f"Modulation depth not linear: R² = {r_squared:.4f}"
    ...
    def test_depth_saturation_and_nonlinear_effects(self):
        ...
        assert (
            variation_spread <= 0.1 * max_variation
...
class TestCombinedModulationPatterns:
    ...
    def test_multiple_modulator_interference_patterns(self):
        ...
        if np.any(peak1_mask) and np.any(peak2_mask):
            ...
            assert (
                peak1_power / total_power >= 0.05
            ), "First modulation frequency not preserved"
            assert (
                peak2_power / total_power >= 0.05
            ), "Second modulation frequency not preserved"
    ...
    def test_temporal_modulation_pattern_coherence(self):
        ...
        # Arrange
        from haptic_system.modulation import AmplitudeModulator, CircularMotionGenerator

        # Create synchronized motion and amplitude modulation
        motion_gen = CircularMotionGenerator(
            rotation_freq=0.25, fluctuation_amplitude=5.0, seed=42  # 4 second rotation
        )
...
class TestModulationSystemIntegration:
    ...
    def test_modulation_parameter_bounds_and_validation(self):
        ...
        for params in valid_amp_params:
            try:
                ...
                pytest.fail(
                    f"Valid amplitude parameters rejected: {params}, error: {e}"
                )
    ...
    def test_modulation_real_time_performance_requirements(self):
        ...
        for block_idx in range(num_blocks):
            ...
            _ = amplitude * np.cos(theta)
            _ = amplitude * np.sin(theta)
