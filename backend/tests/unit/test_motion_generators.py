...
class TestCircularMotionGenerator:
    ...
    def test_generates_correct_rotation_frequency_within_tolerance(self):
        ...
        assert abs(actual_freq - expected_freq) <= tolerance * expected_freq, \
            f"Frequency accuracy failed: expected {expected_freq:.6f} ±{tolerance*100:.1f}%, got {actual_freq:.6f}"
    
    @pytest.mark.parametrize("rotation_freq,expected_period", [
    ...
    def test_rotation_frequency_precision_multiple_values(self, rotation_freq, expected_period):
    ...
    def test_fluctuation_amplitude_stays_within_specified_range(self):
        ...
        generator = CircularMotionGenerator(
            rotation_freq=0.33,
            fluctuation_amplitude=fluctuation_deg,
            seed=42
        )
        ...
        assert max_fluctuation <= safety_margin * fluctuation_rad, \
    ...
    def test_fm_modulation_depth_effect(self):
        ...
        assert std_with_fm > 2 * std_no_fm, \
...
class TestAmplitudeModulator:
    ...
    def test_envelope_frequency_accuracy_within_tolerance(self):
        ...
        # Assert
        assert abs(actual_freq - envelope_freq) <= tolerance * envelope_freq, \
    ...
    def test_amplitude_clipping_boundaries_enforced(self):
        ...
        assert min_amplitude >= expected_min - 1e-6, \
            f"Amplitude below minimum: {min_amplitude:.4f} < {expected_min:.4f}"
        assert max_amplitude <= expected_max + 1e-6, \
            f"Amplitude above maximum: {max_amplitude:.4f} > {expected_max:.4f}"
    
    @pytest.mark.parametrize("envelope_depth,expected_variation_ratio", [
        (0.0, 0.05),   # No envelope - minimal variation
        (0.1, 0.15),   # Small envelope
        (0.25, 0.35),  # Medium envelope  
        (0.3, 0.4),    # Large envelope
    ])
    def test_modulation_depth_effects_amplitude_variation(self, envelope_depth, expected_variation_ratio):
        ...
        assert abs(variation_ratio - expected_variation_ratio) <= tolerance, \
    ...
    def test_combined_envelope_and_noise_modulation(self):
        ...
        assert abs(mean_both - mean_envelope) <= 0.05, \
...
class TestDirectionalFluctuationGenerator:
    ...
    def test_multiple_frequency_components_present(self):
        ...
        generator = DirectionalFluctuationGenerator(
            fluctuation_components=components
        )
        ...
        for comp in components:
            ...
            if np.any(freq_mask):
                ...
                if target_freq == 0.3:  # Use first component as reference
                    ...
                    expected_ratio = expected_relative_power / (reference_comp["amp"]**2)
    ...
    def test_phase_relationships_between_components(self):
        ...
        gen1 = DirectionalFluctuationGenerator(
            fluctuation_components=[components[0]]
        )
        gen2 = DirectionalFluctuationGenerator(
            fluctuation_components=[components[1]]
        )
        ...
        assert abs(abs(shift_samples) - expected_shift) <= shift_tolerance, \
    ...
    def test_amplitude_control_per_component(self):
        ...
        tolerance = 0.1  # 10% tolerance
        assert abs(rms_ratio - amplitude_ratio) <= tolerance * amplitude_ratio, \
...
class TestMotionGeneratorIntegration:
    ...
    def test_circular_motion_with_directional_fluctuations_combined(self):
        ...
        # Arrange
        from haptic_system.modulation import CircularMotionGenerator, DirectionalFluctuationGenerator
        ...
        assert abs(actual_rotations - expected_rotations) <= rotation_tolerance, \
        ...
        assert combined_variation > base_variation, \
    ...
    def test_amplitude_modulation_preserves_vector_magnitude_properties(self):
        ...
        assert abs(mean_amplitude - 1.0) <= 0.1, \
